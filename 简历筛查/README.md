...
### 5. 双模多模型引擎与全链路真大模型动态推理
* **本地离线端侧优先**：启动自检自动优先命中本地 LM Studio（`http://127.0.0.1:1234/v1`，模型推荐 `google/gemma-4-e4b`、`qwen3.8-27b`），保证最终交付在客户本地电脑上 100% 纯离线断网运行；
* **商用云端平滑切换**：顶栏状态胶囊支持一键切换至 **DeepSeek (V3/R1)**、**通义千问 (DashScope)**、**月之暗面 (Kimi)** 等主流模型，自带连通性测速；
* **全链路真实动态推理（彻底消除硬编码与插槽模板）**：
  * **个性化面试题动态生成**：点击【AI换一批追问】彻底告别此前机械的 `【公司】` 和 `【技能】` 槽位硬编码，改由底层 `generate_chat` 将候选人真实战绩细节与目标 JD 注入大模型（本地 LM Studio 或云端 DeepSeek-R1），现场动态推理生成 3 道具备大厂实战攻防深度的面试追问；
  * **沉浸式候选人微聊拟真**：微聊沟通模块全面移除关键词 `if-else` 回复，改由大模型结合候选人具体经历、期望薪资、到岗时间与技术擅长进行多轮拟真人机微聊，全面增强 AI 的现场参与感；
  * **原生支持 `.env` 环境变量弹性注入**：`config.py` 现已原生支持从系统 `.env` 中读取 `DEEPSEEK_API_KEY`、`DEEPSEEK_API_URL`、`AGENT_MODEL`、`ACTIVE_PROVIDER` 作为全局回退与统一调度，使三大业务工程配置高度统一；
  * **深度思考链容错与 `<think>` 标签清洗**：支持针对 DeepSeek-R1 / Qwen 等具备思考链模型的 `<think>` 标记实时清洗与 `reasoning_content` 回收容错，彻底消除大模型结构化输出截断异常。
...
* **访问地址要用回环地址**：服务按 `HOST` 绑定（默认 `0.0.0.0`，供局域网访问），但 `0.0.0.0` 是"监听所有网卡"的**绑定地址、不能当作访问目标**——手动访问请用 **`http://127.0.0.1:8030`**；`run.py` 已按此规则自动拉起浏览器（`BROWSER_HOST`），若显式设了 `HOST=192.168.x.x` 则沿用该值。

### 13. 去伪保真：展示位只写"系统真做过的事"（2026-09-17 确立）

招聘场景要**可解释、可追责**，因此凡是"看起来像智能体真实做过的事"的展示位（执行轨迹、思维链、亮点核实结论、核验徽标），**模型/系统没真做就不写**：

| 展示位 | 现行为 |
|---|---|
| **执行轨迹**（`deep_audit.investigation_trace`） | 初筛主流程是**代码编排的真实骨架**，阶段 3 挂着真实工具返回（时间线测谎 / 外链探查 / 项目含金量）；"重新尽调推测"路径由模型按**实际步骤数**输出（通常 2~5 条，不强制凑四段）。模块标题已注明"系统编排与工具取证，**非模型原始思维链**" |
| **模型未返回结构化轨迹时** | 只显示一条【系统说明】如实告知，**不再用模板编造四条阶段与结论**（旧实现会写出"综合核验无虚假注水…评定为优秀推荐"这类可能与思维链相反的评语） |
| **思维链** | 模型无流式 reasoning 时如实说明"本次不含模型逐步推理"，**不再用模板拼接冒充** |
| **核验类结论** | 不再出现"学历真实 / 履历属实 / 经实名核验 / 企业实名及学历官方双核验"等系统并未执行的断言；统一改为**简历内一致性 + 公开渠道（开源仓库/技术博客）核查**口径 |

### 14. 实名背调：只做状态跟踪，不在初筛阶段执行

实名背调（身份、学历、任职经历）属**敏感个人信息**，依《个人信息保护法》需**告知 + 单独同意**，因此：

```
上传简历 → 系统内一致性核验（时间线/项目含金量/公开外链）
        → AI 初筛评分 → 面试 → ★offer 前★
        → 候选人签《背景调查授权书》→ HR 自办或有资质第三方核验
          （学历=学信网、身份=实名核验、任职=证明人/社保个税）
        → 结果回填 → offer 审批放行
```

* **系统侧只做状态跟踪**（状态 / 渠道 / 授权时间 / 报告编号），**不自己"核验"**、不在初筛阶段声称已核验；
* 本地明文 `data/store.json` **不存身份证号、学信网账号等敏感原件**；
* 预置演示剧本（`presets.py` 与 `rescreen_candidate` 中 preset-001~004 的高保真画像）属**演示样本**，非真实取证。

> 改这类文案时**必须同时改存量数据**：`data/store.json`（运行数据）+ `data/store.baseline.json`（一键恢复的还原源）——只改代码界面不会变（storage 仅在文件缺失时才用 `presets.py` 初始化）。迁移脚本模式：定向替换 + `json.loads` 校验 + 落盘前备份到 `data/backup/*.premigrate.json`，幂等可重跑。
