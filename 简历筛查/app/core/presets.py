...
PRESET_CANDIDATES: List[Dict[str, Any]] = [
    {
        "id": "preset-001",
        "deep_audit": {
            "reasoning_chain": "针对候选人【林远志】的 ReAct 自主交叉尽调推导：\n1. 时序真实性比对：2018年毕业于浙江大学（985计算机硕士），工龄8年。核验字节跳动（2018-2021，3年）与腾讯科技（2021至今，近4年）的时序衔接完全自洽，无虚假跳槽空窗或时间线断层；\n2. 工程含金量与量化脱水：自述微前端沙箱自研与日均千万级流量支撑，深入核实其开源仓库与技术主导细节，代码风格严谨，涉及跨子应用通信、CSS作用域隔离与BFF服务治理，量化得分95分，无注水浮夸；\n3. 岗位匹配度综合反思：候选人在微前端体系、React/Node全栈架构及高并发场景下的工程积淀与本次【资深前端开发/全栈工程师】JD要求100%覆盖，评定为顶尖 S 级。",
            "investigation_trace": [
        ...
        "question_theme": "架构设计与极端高并发攻坚",
        "interview_questions": [
            {
                "thinking": "候选人在腾讯主导过微前端架构，但微前端最棘手的是各子应用的JS全局变量污染、CSS样式溢出及跨域通信性能瓶颈。考查其是否具备深度的自研沙箱内核能力或主流框架（qiankun/wujie）的深度魔改经验。",
                "question": "请结合你在腾讯主导微前端架构的实战经历，详细谈谈微应用间的沙箱隔离与通信机制是如何设计的？"
            },
            {
                "thinking": "候选人负责过高频协同编辑与复杂渲染大屏，React组件树深度往往导致严重的掉帧。考查其Fiber调度机制理解、虚拟长列表、Web Worker分流及SSR多级缓存架构能力。",
                "question": "针对React高并发/大数据量渲染场景，你有哪些具体的性能调优策略及服务端渲染（SSR）缓存架构设计？"
            },
            {
                "thinking": "候选人履历中主导了日均超千万次请求的Node.js BFF层，Node单线程事件循环阻塞是致命痛点。考查其核心Dump排查路径、V8堆内存快照分析工具链及线程池调优水平。",
                "question": "在Node.js微服务中，如何进行系统性内存泄漏排查与高并发事件循环瓶颈优化？"
            }
        ],
        "full_resume": {
            "basic_info": {
