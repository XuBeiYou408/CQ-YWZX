...
echo ====================================================================
echo   RecruitAI · 企业智能招聘与简历初筛工作台 - 正在启动...
echo ====================================================================
echo.
...
if not exist ".venv\Scripts\python.exe" (
    echo [1/3] 首次运行：正在为简历筛查项目创建独立虚拟环境 (.venv)...
    %PY_CMD% -m venv .venv
    if not exist ".venv\Scripts\python.exe" (
        echo.
    ...
    REM 幂等检查：验证核心依赖是否完整 (fastapi, docx, pymupdf, multipart, httpx)
    ".venv\Scripts\python.exe" -c "import fastapi, docx, pymupdf, multipart, httpx" >nul 2>nul
    if errorlevel 1 (
...
REM 3. 启动服务
echo [3/3] 虚拟环境就绪，正在启动 RecruitAI 工作台 (端口 8030)...
echo.
".venv\Scripts\python.exe" run.py

if %errorlevel% neq 0 (
    echo.
    echo [提示] RecruitAI 服务异常中断或已退出。
    pause
)
