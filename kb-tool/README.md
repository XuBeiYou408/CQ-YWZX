...
> ⚠️ **已知限制（与本仓库的 Web 版 `企业本地知识库` 存在差异，待同步修复）**
> 本项目的 `rag/retriever.py` 与 Web 版同源但**尚未同步 2026-09-17 的检索链路修复**，因此：
> 1. **BM25 未接中文分词**——langchain 默认 `text.split()` 会把整句中文切成 1 个 token，
>    该路返回的实际是噪声（自检：`BM25Retriever.from_documents(docs).preprocess_func("请假怎么审批")` 长度为 1 即中招）；
> 2. **双路召回未做 RRF 融合**——`向量 Top-35 + BM25 Top-6` 拼接后直接截断，BM25 命中位次恒在末尾、
>    基本进不了最终上下文（Web 版已改为 RRF 融合 + 中文分词）；
> 3. 未配置 `RERANKER_MODEL_PATH` 时无精排，排序即向量召回首序。
>
> 修复方案已在 Web 版验证（见 `企业本地知识库/README.md` 的「检索链路说明」），移植过来即可，代码同构。
