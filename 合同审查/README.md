...
## ⚡ 性能与调优（2026-09-15 实测标定）

审查耗时由「走哪条管道」和「模型推理速度」共同决定，实测基线：

| 场景 | 链路 | 引擎耗时 | 说明 |
|---|---|---|---|
| 合同 < 3000 字 或 条款 < 8 | **单次管道**（全文一次审查） | **≈80~90 秒** | 不做逐条款深查 |
| 合同 ≥ 3000 字 且 条款 ≥ 8 | **Agent 全链路**（感知→规划→逐条款深查→反思） | **≈5~8 分钟** | 10 条款约 8~9 次模型调用，≈50s/次 |

### 可调项（都在 `config.py`，均支持环境变量覆盖）

| 变量 | 默认 | 作用 | 调整建议 |
|---|---|---|---|
| `AGENT_SMALL_CONTRACT_CHARS` | **3000** | 字数低于此值 → 走单次管道 | 演示求快：保持 3000；求细：调回 2000 |
| `AGENT_SMALL_CONTRACT_CLAUSES` | 8 | 条款数低于此值 → 走单次管道 | 一般不动 |
| `AGENT_MAX_DEEP_DIVE` | **3** | Agent 深查条款数上限 | 每 +1 条 ≈ +50 秒 |
| `AGENT_TIME_BUDGET_SECONDS` | 1500 | Agent 全局时间预算 | 一般不动 |

> ℹ️ **改完开新会话即可生效**（实测：2026-09-15 11:04 新会话已按新阈值走单次管道）。
> 原理：MCP 服务随会话拉起（per-session sidecar），新会话 = 新进程 = 读到新配置；
> 但**在旧会话里继续测试仍是旧阈值**（`config.py` 只在进程启动时读取一次）。
> 若发现没生效，再按"重启 WorkBuddy / 对 `contract-reviewer` 取消信任→重新信任"兜底。
...
| 模型 | 大小 | 64K 显存预估 | 工具调用形状正确率 | 单次响应 | 结论 |
|---|---|---|---|---|---|
| `google/gemma-4-e4b` | 7.5B(激活4B) | 9.63 GiB | **0/3**（写错层级 + 反复重试） | 20~35s | ❌ 不适合做工具调用 |
| **`qwen/qwen3.5-9b`** | 9B | **10.04 GiB** | **3/3** ✔ | **9.7~13.3s** | ✅ **当前推荐** |
...
```bat
"C://Users//ASUS//.lmstudio//bin//lms.exe" unload --all
"C://Users//ASUS//.lmstudio//bin//lms.exe" load "qwen/qwen3.5-9b" -c 65536 --parallel 1 -y
```
- **WorkBuddy 的对话模型也请切到 `qwen/qwen3.5-9b`**，保持"同一时刻只加载一个模型"（两个大模型并存会因内存紧张被自动卸载，导致工具调用中途失败）。
- 若想固定审查所用模型（不随会话模型变化）：设环境变量 `AGENT_MODEL=qwen/qwen3.5-9b`。
