...
功能（全部幂等，可重复运行）：
  1. 检查/创建项目 venv 并安装依赖（requirements.txt），并做 mcp 版本兼容性体检
     （2.x 移除了 mcp.server.fastmcp，会致服务 import 即崩，检测到不兼容会自动降级修复）
  2. 把 contract-reviewer 注册进 WorkBuddy 的 mcp.json（合并，不动其他服务）
  3. 把 deploy/skills/contract-review 部署为用户级 Skill（~/.workbuddy/skills/）
  4. 向 ~/.workbuddy/MEMORY.md 写入合同审查路由规则（带标记，幂等 upsert）
  5. 探测本机 LM Studio（非致命）+ 检查 WorkBuddy 信任状态
  6. 端到端体检（真实启动本项目的 MCP 服务，协议握手计时、列出工具、调用 ping 环境检测）
